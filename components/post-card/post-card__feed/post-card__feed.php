...
// если прилетела строка (URL/ID), не даём упасть
    ...
    <? 
    // Приоритет: post_card_link > feed_link > обычная ссылка
    $final_link_url = null;
    $final_link_target = '_self';
    
    if ($post_card_link && !empty($post_card_link['url'])) {
        $final_link_url = $post_card_link['url'];
        $final_link_target = $post_card_link['target'] ? $post_card_link['target'] : '_self';
    } elseif ($feed_link && !empty($feed_link['url'])) {
        $final_link_url = $feed_link['url'];
        $final_link_target = $feed_link['target'] ? $feed_link['target'] : '_self';
    }
    ?>
    <? if ($final_link_url): ?>
        <a class="post-card-heading" href="<?php echo esc_url($final_link_url); ?>" target="<?php echo esc_attr($final_link_target); ?>">
            <?= $title ?>
